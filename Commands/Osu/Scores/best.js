...
async function get_osuScores_Best(id, gamemode, limit, offset) {
    let request, error;
...
module.exports = {
	...
	async execute(interaction) {
        ...
        const { request: scores, error: scoresError } = await get_osuScores_Best(user.id, gamemode || user.playmode, limit, offset);
        ...
        const bestScoresEmbed = new EmbedBuilder()
            .setColor("#FAD39E")
            .setAuthor({
        ...
        await interaction.editReply({
            content: "",
            embeds: [bestScoresEmbed.setDescription(scoresList)]
        });
	}
};